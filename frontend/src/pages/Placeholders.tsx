...
export function EmployeePage() {
 const [profile, setProfile] = useState<EmployeeProfile>(); useEffect(() => { void careerQuestApi.employee('E0028').then(setProfile) }, [])
 ...
 return <section className="page"><p className="eyebrow">Мой карьерный путь</p><h1>{employee.full_name}</h1><div className="profile-grid"><article className="panel"><h2>{employee.role} · {employee.grade}</h2><p>{employee.department} · {employee.tenure_months} месяцев в компании</p><p>Цель: <b>{employee.career_goal?.target_role} {employee.career_goal?.target_grade}</b></p><p className={next_grade.requirements_met ? 'success' : 'warning'}>Траектория: {next_grade.grade}</p></article><article className="panel chart"><Radar skills={skills}/><small>Синий — текущий уровень, золотой контур — требуемый.</small></article></div><h2>Навыки для следующего уровня</h2><div className="skills">{skills.map(s => <article className="skill" key={s.skill_id}><div><b>{s.name}</b>{s.critical && <em>Критичный</em>}<small>{s.current}/5 сейчас · требуется {s.required}/5</small></div><div className="meter"><i style={{width:`${s.current*20}%`}}/><strong style={{left:`${s.required*20}%`}}/></div></article>)}</div><Recommendations /><h2>История активностей</h2>{history.length ? <ul>{history.map(r=><li key={r.record_id}>{r.title} — {r.status}</li>)}</ul> : <p className="muted">Пока нет завершённых активностей.</p>}</section>
}
export function HrPage() {
  return <section className="page"><p className="eyebrow">Только для HR</p><h1>HR-дашборд</h1><div className="dashboard"><article className="panel"><h2>Проседающие навыки</h2><p><b>System Design</b> · средний разрыв 1.8</p><p><b>Product Thinking</b> · средний разрыв 1.2</p></article><article className="panel"><h2>Без рекомендации</h2><p>3 сотрудника требуют внимания</p><small>Только агрегаты: личная вовлечённость скрыта.</small></article><article className="panel"><h2>Участие в активностях</h2><p>Завершение: <b>76%</b></p><p>Drop-rate: <b>8%</b></p></article></div></section>
}
